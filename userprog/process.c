...
process_execute (const char *file_name) 
{
  ...
  while(file_name_[i] != '\0' && file_name_[i] != ' ') i++;
  file_name_[i]='\0';
	check("parsing in process_execute", CHECKMARK);
...
//	printf("thread name : %s, thread tid %d\n", t->name, tid);

	//project_2
	sema_down(&t->sync);

	struct thread *child;
	struct list_elem *elem=list_begin(&(t->child));

	
	while(elem!=list_end(&(t->child))){
		child=list_entry(elem, struct thread, child_elem);
		if(child->tid==-1){
			check("wrong open thread", CHECKMARK);
			return -1;
		}
		elem=list_next(elem);
	}
...
//project_2
   if (!success){
		check("wrong open", CHECKMARK);
		t->tid=-1;
		sema_up(&t->parent->sync);
		exit(-1);
	}
	else{
		check("open well", CHECKMARK);
		sema_up(&t->parent->sync);
	}
...
process_wait (tid_t child_tid) 
{
	...
	while(elem!=list_end(&(t->child))){
		...
		if(child_tid == child->tid){
			...
			check("get child status", CHECKMARK);
			exit_status=child->exit_status;
			sema_up(&(child->done));
			check("child finish", CHECKMARK);
...
check("argument parsing in load", CHECKMARK);
  /*argument parsing*/
 // printf("hi\n");
  char *argv[150], *str;
  char *token=strtok_r(file_name," ",&str);
  int index=0;
  while(token!=NULL){
	argv[index++]=token;
